...
| VMware GETTIME backdoor | [`vmware-gettime`](https://github.com/felixrieseberg/v86/tree/vmware-gettime) | — *(stacked on the clipboard branch)* | ✅ | Win95 only reads the RTC at boot, so a resumed state keeps a stale clock; `W95TOOLS.EXE` polls GETTIME (command 23) and steps the clock back to host time. |
